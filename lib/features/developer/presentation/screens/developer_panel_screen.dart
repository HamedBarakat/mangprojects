...
class DeveloperPanelScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DeveloperPanelScreen> createState() => _DeveloperPanelScreenState();
}
    ...
    return Scaffold(
      ...
      floatingActionButton: _tabs.index == 0
          ? FloatingActionButton.extended(
              ...
              label: const Text('New Office', style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      builder: (context, snap) {
        ...
        if (offices.isEmpty) {
          return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const Text('No offices yet', style: TextStyle(color: Colors.white38)),
            const SizedBox(height: 6),
            const Text('Tap + to create the first office',
...
class _OfficeCard extends ConsumerWidget {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child: Row(children: [
            Icon(Icons.access_time_rounded, size: 13, color: Colors.white38),
            const SizedBox(width: 4),
            Text('${office.workStartTime} – ${office.workEndTime}',
                style: const TextStyle(color: Colors.white38, fontSize: 11)),
            const Spacer(),
            if (office.createdAt != null)
              Text(_fmtDate(office.createdAt!),
  ...
  Future<void> _activate(BuildContext context, WidgetRef ref, DeveloperRepository repo) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => _DevDialog(
        title: 'Activate Office',
        message: 'Activate "${office.name}"? Users will be able to log in again.',
  ...
  Future<void> _suspend(BuildContext context, WidgetRef ref, DeveloperRepository repo) async {
    final reasonCtrl = TextEditingController();
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        backgroundColor: const Color(0xFF111E35),
        title: const Text('Suspend Office', style: TextStyle(color: Colors.white)),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('Suspend "${office.name}"?\nAll users will be locked out.',
    ...
    if (confirm == true) {
      await repo.toggleOfficeStatus(
        officeId: office.id, isActive: false, suspendReason: reasonCtrl.text.trim(),
      );
    }
  }
      ...
      if (bytes.lengthInBytes > 800 * 1024) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Image too large. Please choose a smaller image (< 800KB).'),
  ...
  Future<void> _removeLogo() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Are you sure you want to remove the office logo?',
    ...
    return Row(children: [
      ...
      Container(
        ...
        child: hasLogo
            ...
            : const Icon(Icons.image_outlined, color: Colors.white24, size: 18),
      ),
      ...
      Expanded(
        child: _selectedOffice == null
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const Text('Select an office above', style: TextStyle(color: Colors.white38)),
              ]))
...
class _UsersListState extends ConsumerState<_UsersList> {
  ...
  Future<void> _loadUsers() async {
    ...
    final users = await ref.read(developerRepositoryProvider)
  ...
  Widget build(BuildContext context) {
    ...
    if (users.isEmpty) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const Text('No users found', style: TextStyle(color: Colors.white38)),
      ]));
    }
    return ListView.separated(
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          width: 40, height: 40,
          decoration: BoxDecoration(
            color: isAdmin ? Colors.amber.withOpacity(0.12) : Colors.white.withOpacity(0.06),
            shape: BoxShape.circle,
          ),
          child: Icon(
            isAdmin ? Icons.admin_panel_settings_rounded : Icons.person_outline_rounded,
            color: isAdmin ? Colors.amber : Colors.white54, size: 20,
          ),
        ),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            if (isAdmin) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.amber.withOpacity(0.12), borderRadius: BorderRadius.circular(4),
                ),
...
class _CreateOfficeSheetState extends ConsumerState<_CreateOfficeSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await ref.read(developerRepositoryProvider).createOffice(
        ...
        Flexible(
          child: SingleChildScrollView(
            ...
            child: Form(
              key: _formKey,
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Create New Office',
                    style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                const SizedBox(height: 20),

                // Office info
                _DevLabel('Office Name'),
                _DevField(controller: _nameCtrl, hint: 'e.g. Afaq Engineering',
                ...
                Row(children: [
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(
                      color: Colors.amber.withOpacity(0.12), borderRadius: BorderRadius.circular(6),
                    ),
                ...
                _DevField(controller: _adminNameCtrl, hint: 'Full name',
                    validator: (v) => v!.isEmpty ? 'Required' : null),
                const SizedBox(height: 12),

                _DevLabel('Admin Email'),
                _DevField(
                  controller: _adminEmailCtrl, hint: 'admin@company.com',
                ...
                _DevField(
                  controller: _adminPassCtrl, hint: 'Min 6 characters',
                ...
                SizedBox(
                  ...
                  child: FilledButton(
                    ...
                    child: _saving
                        ...
                        : const Text('Create Office + Admin',
...
class _EditUserSheetState extends ConsumerState<_EditUserSheet> {
  ...
  Future<void> _saveEmail() async {
    ...
    setState(() => _saving = true);
    try {
      await ref.read(developerRepositoryProvider).updateUserEmail(
        uid: widget.user.uid, newEmail: newEmail,
      );
      if (mounted) {
        widget.onSaved();
        Navigator.pop(context);
  ...
  Future<void> _sendPasswordReset() async {
    setState(() => _saving = true);
    try {
      await ref.read(developerRepositoryProvider).sendPasswordReset(widget.user.email);
      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(alignment: Alignment.center, margin: const EdgeInsets.only(bottom: 16),
              child: Container(width: 40, height: 4,
                  decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(2)))),

          Text('Edit User — ${widget.user.name}',
          ...
          SizedBox(
            ...
            child: FilledButton.icon(
              ...
              label: const Text('Save Email', style: TextStyle(fontWeight: FontWeight.bold)),
