...
import '../../../../features/projects/data/models/task_model.dart' show TaskModel;
...
final _projectDetailProvider = StreamProvider.family<Map<String, dynamic>?, String>(
  (ref, projectId) => FirebaseFirestore.instance
      .collection('projects')
      .doc(projectId)
      .snapshots()
...
class ClientProjectDetailScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ClientProjectDetailScreen> createState() => _ClientProjectDetailScreenState();
}

class _ClientProjectDetailScreenState extends ConsumerState<ClientProjectDetailScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final progress = (project?['completionPercentage'] as num?)?.toDouble() ?? 0.0;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          if (project != null) _ProjectHeader(project: project, progress: progress, status: status),
          Expanded(
            child: TabBarView(
              controller: _tabs,
              children: [
                _TasksTab(projectId: widget.projectId, clientId: widget.clientId),
                _NotesTab(projectId: widget.projectId, clientId: widget.clientId),
                _ReportTab(
...
class _ProjectHeader extends StatelessWidget {
  ...
  final String status;
  const _ProjectHeader({required this.project, required this.progress, required this.status});
      ...
      child: Column(children: [
        Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            if (project['endDate'] != null)
              Text('End: ${_fmtDate(project['endDate'])}',
        ...
        ClipRRect(
          ...
          child: LinearProgressIndicator(
            value: progress / 100, minHeight: 10,
...
class _TasksTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return tasksAsync.when(
      ...
      data: (tasks) {
        if (tasks.isEmpty) return _emptyState(context, Icons.task_alt_outlined, 'No tasks yet');
        return ListView.separated(
          padding: const EdgeInsets.all(16),
          itemCount: tasks.length,
          separatorBuilder: (_, __) => const SizedBox(height: 10),
          itemBuilder: (ctx, i) => _TaskCard(task: tasks[i], clientId: clientId),
        );
      },
    );
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child: Row(children: [
            Container(width: 10, height: 10,
                decoration: BoxDecoration(color: _taskStatusColor(task.status), shape: BoxShape.circle)),
        ...
        Padding(
          ...
          child: Row(children: [
            Icon(Icons.person_outline, size: 13, color: cs.onSurface.withOpacity(0.4)),
            const SizedBox(width: 4),
            Text(task.assignedToName.isEmpty ? '—' : task.assignedToName,
                style: TextStyle(fontSize: 11, color: cs.onSurface.withOpacity(0.55))),
            const SizedBox(width: 14),
            Icon(Icons.calendar_today_outlined, size: 13, color: cs.onSurface.withOpacity(0.4)),
            const SizedBox(width: 4),
            Text(_fmtDateTime(task.endDate),
                style: TextStyle(fontSize: 11, color: cs.onSurface.withOpacity(0.55))),
            const Spacer(),
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: cs.primaryContainer.withOpacity(0.4), borderRadius: BorderRadius.circular(20),
                ),
                child: Row(children: [
                  Icon(Icons.comment_outlined, size: 13, color: cs.primary),
                  const SizedBox(width: 4),
                  Text(comments > 0 ? '$comments' : 'Note',
  ...
  void _showCommentSheet(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (_) => _AddCommentSheet(
        ...
        clientId: clientId, existingComments: task.clientComments,
      ),
    );
  }
}
...
class _TaskStatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = switch (status) {
      ...
      'under_review' => ('Under Review', Colors.indigo),
...
class _CommentsPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isResolved ? Colors.green.withOpacity(0.08) : cs.primaryContainer.withOpacity(0.2),
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: isResolved ? Colors.green.withOpacity(0.3) : Colors.transparent),
      ),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Icon(isResolved ? Icons.check_circle_outline : Icons.format_quote_rounded,
            size: 16, color: isResolved ? Colors.green : cs.primary.withOpacity(0.6)),
        const SizedBox(width: 6),
        Expanded(child: Text(last['text'] as String? ?? '',
...
class _AddCommentSheet extends StatefulWidget {
  ...
  const _AddCommentSheet({
    ...
    required this.clientId, required this.existingComments,
  });
...
class _AddCommentSheetState extends State<_AddCommentSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final newComment = {
        'clientId': widget.clientId, 'text': text,
        'createdAt': Timestamp.now(), 'isRead': false, 'isResolved': false,
      };
      await FirebaseFirestore.instance.collection('tasks').doc(widget.taskId)
          .update({'clientComments': [...widget.existingComments, newComment]});
      if (mounted) {
        _ctrl.clear();
  ...
  Future<void> _toggleResolved(int index) async {
    ...
    try {
      final updated = widget.existingComments.map((c) => Map<String, dynamic>.from(c)).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: cs.surface, borderRadius: const BorderRadius.vertical(top: Radius.circular(24)),
      ),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Notes', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: cs.onSurface)),
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Padding(
                      ...
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        GestureDetector(
                          onTap: _resolving ? null : () => _toggleResolved(i),
                          child: Container(
                            width: 24, height: 24,
                            decoration: BoxDecoration(
                        ...
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(c['text'] as String? ?? '',
                              style: TextStyle(fontSize: 13,
                                color: isResolved ? cs.onSurface.withOpacity(0.4) : cs.onSurface,
                                decoration: isResolved ? TextDecoration.lineThrough : null)),
                          const SizedBox(height: 2),
                          Row(children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                filled: true, fillColor: cs.surfaceContainerHighest.withOpacity(0.5),
...
class _NotesTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return tasksAsync.when(
      ...
      error: (e, _) => Center(child: Text('$e')),
      data: (tasks) {
        final tasksWithNotes = tasks.where((t) => t.clientComments.isNotEmpty).toList();
        ...
        return ListView(
          ...
          children: [
            ...
            ...tasksWithNotes.map((task) => _NoteGroup(
              taskTitle: task.title, taskId: task.id, comments: task.clientComments,
            )),
          ],
        );
      },
...
class _NoteGroup extends StatelessWidget {
  ...
  Future<void> _toggleResolved(int index) async {
    ...
    await FirebaseFirestore.instance.collection('tasks').doc(taskId)
        .update({'clientComments': updated});
  }
          ...
          child: Row(children: [
            ...
            if (resolvedCount > 0) ...[
              Container(
                ...
                child: Text('$resolvedCount resolved',
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Container(
                  ...
                  child: Text(c['text'] as String? ?? '',
                      style: TextStyle(fontSize: 13,
                        color: isResolved ? cs.onSurface.withOpacity(0.45) : cs.onSurface,
                        decoration: isResolved ? TextDecoration.lineThrough : null)),
                ),
                const SizedBox(height: 4),
                Row(children: [
...
class _ReportTab extends StatefulWidget {
  ...
  final Map<String, dynamic>? project;
  const _ReportTab({
    required this.projectId, required this.clientId,
    required this.project, required this.projectName,
  });
...
class _ReportTabState extends State<_ReportTab> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _exportPdf(List<TaskModel> tasks) async {
    ...
    try {
      final progress      = (widget.project?['completionPercentage'] as num?)?.toDouble() ?? 0.0;
      ...
      final review        = tasks.where((t) => t.status == 'under_review').length;
      ...
      final boldStyle  = pw.TextStyle(font: cairoFont, fontSize: 10, fontWeight: pw.FontWeight.bold);
      final titleStyle = pw.TextStyle(font: cairoFont, fontSize: 18, fontWeight: pw.FontWeight.bold,
      ...
      final headerCellStyle = pw.TextStyle(font: cairoFont, fontWeight: pw.FontWeight.bold,
      ...
      pdf.addPage(pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.Text('Client Project Report  •  Generated: ${_fmtNow()}',
              style: pw.TextStyle(font: cairoFont, fontSize: 9, color: PdfColors.grey600)),
          ...
          pw.Container(
            ...
            child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
              ...
              pw.Text('Progress: ${progress.toStringAsFixed(0)}%',
                  style: pw.TextStyle(font: cairoFont, fontSize: 10, fontWeight: pw.FontWeight.bold,
                      color: const PdfColor.fromInt(0xFF2E7D32))),
            ]),
          ),
          ...
          pw.Table.fromTextArray(
            ...
            data: [
              ...
              ['Under Review', '$review',    total > 0 ? '${(review / total * 100).toStringAsFixed(0)}%' : '0%'],
          ...
          pw.Table.fromTextArray(
            ...
            data: tasks.map((t) {
              ...
              return [
                t.title,
                t.assignedToName.isEmpty ? '—' : t.assignedToName,
  ...
  Future<void> _exportExcel(List<TaskModel> tasks) async {
    ...
    try {
      ...
      final summaryData = [
        ...
        ['Under Review', '${tasks.where((t) => t.status == 'under_review').length}'],
      ...
      for (var i = 0; i < summaryData.length; i++) {
        _excelRow(summary, summaryData[i].map((v) => TextCellValue(v)).toList(), i + 1);
      }
      summary.setColumnWidth(0, 22);
      summary.setColumnWidth(1, 28);

      // ── Sheet 2: Tasks ────────────────────────────────────────────────
      // إنشاء sheet جديدة مستقلة بدل copy
      excel['Tasks']; // creates fresh sheet
      final tasksSheet = excel['Tasks'];
      _excelHeader(tasksSheet, ['Task', 'Assigned To', 'Status', 'End Date', 'Total Notes', 'Resolved Notes']);
      for (var i = 0; i < tasks.length; i++) {
        ...
        _excelRow(tasksSheet, [
          ...
          TextCellValue(t.assignedToName.isEmpty ? '—' : t.assignedToName),
      ...
      tasksSheet.setColumnWidth(2, 16);
      ...
      // ── Sheet 3: Notes ────────────────────────────────────────────────
      // إنشاء sheet جديدة مستقلة بدل copy
      excel['Notes']; // creates fresh sheet
            ...
            return ListView(
              ...
              children: [
                ...
                _ReportSection(
                  ...
                  child: Column(children: [
                    Row(children: [
                      ...
                      _CircularProgress(value: progress / 100, color: cs.primary),
                    ]),
                    const SizedBox(height: 12),
                    ClipRRect(
                      borderRadius: BorderRadius.circular(8),
                ...
                _ReportSection(
                  ...
                  child: Column(children: [
                    Row(children: [
                      Expanded(child: _StatCard(label: 'Total', value: '$total', color: cs.primary)),
                      const SizedBox(width: 8),
                      Expanded(child: _StatCard(label: 'Done', value: '$completed', color: Colors.green)),
                    ]),
                    const SizedBox(height: 8),
                    Row(children: [
                      Expanded(child: _StatCard(label: 'In Progress', value: '$inProg', color: Colors.orange)),
                      const SizedBox(width: 8),
                      Expanded(child: _StatCard(label: 'In Review', value: '$review', color: Colors.indigo)),
                      const SizedBox(width: 8),
                      Expanded(child: _StatCard(label: 'Not Started', value: '$notStart', color: Colors.grey)),
                    ]),
                    if (total > 0) ...[
                      const SizedBox(height: 14),
                      _StatusBar(segments: [
                        if (completed > 0) _Segment('Done', completed / total, Colors.green),
                        if (inProg > 0)    _Segment('In Progress', inProg / total, Colors.orange),
                        if (review > 0)    _Segment('Review', review / total, Colors.indigo),
                        if (notStart > 0)  _Segment('Not Started', notStart / total, Colors.grey.shade400),
                      ]),
                    ],
                  ]),
                ),
                ...
                _ReportSection(
                  ...
                  child: totalNotes == 0
                      ...
                      : Column(children: [
                          Row(children: [
                            Expanded(child: _StatCard(label: 'Total', value: '$totalNotes', color: cs.primary)),
                            const SizedBox(width: 8),
                            Expanded(child: _StatCard(label: 'Open', value: '$openNotes', color: Colors.orange)),
                            const SizedBox(width: 8),
                            Expanded(child: _StatCard(label: 'Resolved', value: '$resolvedNotes', color: Colors.green)),
                          ]),
                          if (totalNotes > 0) ...[
                            const SizedBox(height: 14),
                            _StatusBar(segments: [
                              if (resolvedNotes > 0) _Segment('Resolved', resolvedNotes / totalNotes, Colors.green),
                              if (openNotes > 0)     _Segment('Open', openNotes / totalNotes, Colors.orange),
                            ]),
                          ],
                        ]),
                ),
                        ...
                        return Padding(
                          ...
                          child: Row(children: [
                            Container(width: 8, height: 8,
                                decoration: BoxDecoration(color: _taskStatusColor(t.status), shape: BoxShape.circle)),
                            const SizedBox(width: 10),
                            ...
                            if (tNotes > 0)
                              Container(
                                ...
                                child: Text(
                                  tResolved == tNotes ? '$tNotes ✓' : '$tResolved/$tNotes',
                                  style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600,
                                      color: tResolved == tNotes ? Colors.green : Colors.orange),
                                ),
                              ),
                            const SizedBox(width: 6),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: cs.primaryContainer.withOpacity(0.5), borderRadius: BorderRadius.circular(8),
            ),
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ClipRRect(
        ...
        child: SizedBox(
          height: 12,
          child: Row(children: segments.map((s) => Expanded(
            flex: (s.fraction * 100).round(),
...
String _taskStatusLabel(String? s) =>
    const {'not_started': 'Not Started', 'in_progress': 'In Progress',
           'under_review': 'Under Review', 'completed': 'Completed'}[s] ?? (s ?? '—');
